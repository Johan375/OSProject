...
7. Run the command **free -h** . ( @Johan375 ➜ /workspaces/OSProject (main) $ free -h
...
1. Are files in the container persistent. Why not?. ( The files doesn't persist because the container no longer exist )
2. Can we run two, or three instances of debian linux? . ( Yes )
...
1. Check the permission of the files created in myroot, what user and group is the files created in docker container on the host virtual machine? . ( User : root ) , ( Group : root )
